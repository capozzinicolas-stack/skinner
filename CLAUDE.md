...
- **Bug fix (2026-05-06): admin panel ignored custom plan label/price.** First paying customer (Paulo H Bacci, R$ 445/mo Plano Custom Beta) showed up as "Enterprise · R$ 3.299" in `/admin/tenants` and `/admin/tenants/[id]`. Three places needed updating to mirror the B2B-side override pattern already in `billing.ts`: (1) `admin.tenantDetail` now builds `effectivePlanForBill` with `customMonthlyPriceBRL` before calling `calculateMonthlyBill`, (2) admin tenant detail page renders `tenant.planLabel ?? planLabels[tenant.plan]`, (3) admin tenant list page does the same. `calculateMonthlyBill` itself stays unchanged — keeping the override responsibility at the caller is the established pattern. **Stripe charged the correct R$ 445 — the bug was display-only**, no overcharge to refund.
- **Bug fix (2026-05-06): admin dashboard MRR overstated.** Same root cause as the panel display bug above. The `admin.stats` reducer at `admin.ts:60` summed `plan.monthlyPriceBRL` for every active tenant, ignoring `customMonthlyPriceBRL`. With one custom tenant (Paulo R$ 445 on enterprise tier) MRR showed R$ 5.298 instead of the actual R$ 2.444 — overstating real revenue by 2.2x, which would have skewed any board / investor reporting. Fix: include `customMonthlyPriceBRL` in the tenant select and apply the standard `?? plan.monthlyPriceBRL ?? 0` fallback chain in the reduce.
