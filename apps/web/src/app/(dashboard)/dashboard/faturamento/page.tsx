"use client";

import { useState } from "react";
import { trpc } from "@/lib/trpc/client";
import { OrganizationTabs } from "@/components/shared/organization-tabs";
import { useI18n } from "@/lib/i18n/client";

export default function BillingPage() {
  const { t } = useI18n();
  const utils = trpc.useUtils();
  const billing = trpc.billing.status.useQuery();
  const plans = trpc.billing.plans.useQuery();
  const [checkoutLoading, setCheckoutLoading] = useState<string | null>(null);
  const changePlan = trpc.billing.changePlan.useMutation({
    onSuccess: () => {
      utils.billing.status.invalidate();
      utils.dashboard.stats.invalidate();
    },
  });

  async function handleStripeCheckout(planId: string) {
    setCheckoutLoading(planId);
    try {
      const res = await fetch("/api/billing/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ planId }),
      });
      const data = await res.json();
      if (data.url) {
        window.location.href = data.url;
      } else {
        alert(data.error || "Erro ao criar sessao de pagamento");
        setCheckoutLoading(null);
      }
    } catch {
      alert("Erro ao conectar com o sistema de pagamento");
      setCheckoutLoading(null);
    }
  }
  const usageHistory = trpc.billing.usageHistory.useQuery();

  if (billing.isLoading) return <div className="p-8 text-pierre font-light">{t.dashboardPages.common_loading}</div>;
  if (!billing.data) return null;

  const b = billing.data;

  return (
    <>
      <OrganizationTabs />
    <div className="p-4 md:p-8 max-w-4xl">
      <h1 className="font-serif text-xl md:text-2xl text-carbone">{t.dashboardPages.bill_title}</h1>
      <p className="text-pierre text-sm font-light mt-1">
        {t.dashboardPages.bill_subtitle}
      </p>

      {/* Current plan + usage */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
        <div className="p-6 bg-white border border-sable/20">
          <p className="text-[10px] text-pierre uppercase tracking-wider font-light">{t.dashboardPages.bill_current_plan}</p>
          <p className="text-2xl font-serif text-carbone mt-2">{b.planName}</p>
          {b.currentBill.baseFee > 0 && (
            <p className="text-sm text-pierre font-light mt-1">
              R$ {b.currentBill.baseFee.toLocaleString("pt-BR")}{t.dashboardPages.bill_per_month}
            </p>
          )}
        </div>
        <div className="p-6 bg-white border border-sable/20">
          <p className="text-[10px] text-pierre uppercase tracking-wider font-light">{t.dashboardPages.bill_credits}</p>
          <p className="text-2xl font-serif text-carbone mt-2">
            {b.analysisUsed} / {b.analysisLimit}
          </p>
          <div className="w-full h-1 bg-sable/20 mt-3">
            <div
              className="h-full bg-carbone transition-all"
              style={{ width: `${Math.min((b.analysisUsed / b.analysisLimit) * 100, 100)}%` }}
            />
          </div>
          <p className="text-xs text-pierre font-light mt-1">
            {t.dashboardPages.bill_credits_remaining.replace("{count}", String(b.creditsRemaining))}
          </p>
        </div>
        <div className="p-6 bg-white border border-sable/20">
          <p className="text-[10px] text-pierre uppercase tracking-wider font-light">{t.dashboardPages.bill_estimated}</p>
          <p className="text-2xl font-serif text-carbone mt-2">
            R$ {b.currentBill.total.toFixed(2)}
          </p>
          <div className="text-xs text-pierre font-light mt-2 space-y-0.5">
            <p>{t.dashboardPages.bill_base}: R$ {b.currentBill.baseFee.toFixed(2)}</p>
            {b.currentBill.excessCost > 0 && (
              <p>{t.dashboardPages.bill_excess.replace("{count}", String(b.currentBill.excessAnalyses))}: R$ {b.currentBill.excessCost.toFixed(2)}</p>
            )}
            {b.currentBill.commissionCost > 0 && (
              <p>{t.dashboardPages.bill_commission.replace("{pct}", (b.commissionRate * 100).toFixed(0))}: R$ {b.currentBill.commissionCost.toFixed(2)}</p>
            )}
          </div>
        </div>
      </div>

      {/* Plans comparison — hidden for custom-plan tenants since their
          plan is bespoke and the standard tiers do not represent options
          they can self-serve switch to. */}
      {b.isCustomPlan ? (
        <div className="mt-12 p-6 bg-ivoire border border-sable/30">
          <p className="text-sm text-carbone font-light">
            {t.dashboardPages.bill_custom_plan_title.replace("{name}", b.planName)}
          </p>
          <p className="text-xs text-pierre font-light mt-2">
            {t.dashboardPages.bill_custom_plan_contact}
          </p>
        </div>
      ) : (
      <div className="mt-12">
        <h2 className="font-serif text-lg text-carbone mb-4">{t.dashboardPages.bill_available_plans}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.data?.map((plan) => {
            const isCurrent = plan.id === b.plan;
            return (
              <div
                key={plan.id}
                className={`p-6 bg-white border ${isCurrent ? "border-carbone" : "border-sable/20"}`}
              >
                <p className="text-[10px] text-pierre uppercase tracking-wider font-light">
                  {isCurrent ? t.dashboardPages.bill_current_plan : "\u00A0"}
                </p>
                <h3 className="font-serif text-xl text-carbone mt-1">{plan.name}</h3>
                <p className="text-lg text-carbone mt-2">
                  {plan.monthlyPrice ? `R$ ${plan.monthlyPrice.toLocaleString("pt-BR")}` : t.dashboardPages.bill_on_request}
                  {plan.monthlyPrice && <span className="text-xs text-pierre font-light">{t.dashboardPages.bill_per_month}</span>}
                </p>
                {plan.setupFee && (
                  <p className="text-xs text-pierre font-light">
                    {t.dashboardPages.bill_setup_label.replace("{price}", plan.setupFee.toLocaleString("pt-BR"))}
                  </p>
                )}
                <ul className="mt-4 space-y-1.5">
                  {plan.features.map((f, i) => (
                    <li key={i} className="text-xs text-pierre font-light">{f}</li>
                  ))}
                </ul>
                {!isCurrent && plan.monthlyPrice && (
                  <button
                    onClick={() => handleStripeCheckout(plan.id)}
                    disabled={checkoutLoading === plan.id}
                    className="w-full mt-6 py-2.5 bg-carbone text-blanc-casse text-xs font-light tracking-wide hover:bg-terre disabled:opacity-50 transition-colors"
                  >
                    {checkoutLoading === plan.id ? t.dashboardPages.bill_redirecting : t.dashboardPages.bill_subscribe}
                  </button>
                )}
                {isCurrent && (
                  <div className="w-full mt-6 py-2.5 text-center text-xs text-pierre font-light border border-sable/30">
                    {t.dashboardPages.bill_active}
                  </div>
                )}
              </div>
            );
          })}
        </div>
      </div>
      )}

      {/* Usage history */}
      {usageHistory.data && usageHistory.data.length > 0 && (
        <div className="mt-12">
          <h2 className="font-serif text-lg text-carbone mb-4">{t.dashboardPages.bill_history}</h2>
          <div className="bg-white border border-sable/20 overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-sable/20 bg-ivoire/50">
                  <th className="text-left px-5 py-3 text-[10px] text-pierre uppercase tracking-wider font-light">{t.dashboardPages.common_date}</th>
                  <th className="text-left px-5 py-3 text-[10px] text-pierre uppercase tracking-wider font-light">{t.dashboardPages.bill_th_type}</th>
                  <th className="text-left px-5 py-3 text-[10px] text-pierre uppercase tracking-wider font-light">{t.dashboardPages.bill_th_qty}</th>
                  <th className="text-right px-5 py-3 text-[10px] text-pierre uppercase tracking-wider font-light">{t.dashboardPages.bill_th_value}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-sable/10">
                {usageHistory.data.slice(0, 20).map((event) => (
                  <tr key={event.id} className="hover:bg-ivoire/30">
                    <td className="px-5 py-3 text-sm text-carbone font-light">
                      {new Date(event.createdAt).toLocaleDateString("pt-BR")}
                    </td>
                    <td className="px-5 py-3 text-sm text-carbone font-light">
                      {event.type === "analysis" ? t.dashboardPages.bill_type_analysis : event.type === "commission" ? t.dashboardPages.bill_type_commission : event.type}
                    </td>
                    <td className="px-5 py-3 text-sm text-pierre font-light">{event.quantity}</td>
                    <td className="px-5 py-3 text-sm text-carbone font-light text-right">
                      {event.total ? `R$ ${event.total.toFixed(2)}` : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      )}
    </div>
    </>
  );
}
