...
export default function LeadsPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      {leads.data && leads.data.length > 0 && (
        <>
          {/* Mobile: stacked cards. Tables don't survive 360-400px viewports
              with 6 columns — too much horizontal scroll for finger flicks. */}
          <div className="md:hidden space-y-3">
            {leads.data.map((lead) => {
              const skin = lead.skinType ? tr(skinTypeLabels, lead.skinType) : null;
              const obj = lead.primaryObjective ? tr(objectiveLabels, lead.primaryObjective) : null;
              return (
                <div key={lead.id} className="bg-white border border-sable/20 p-4 space-y-3">
                  ...
                  {(lead.clientEmail || lead.clientPhone) && (
                    <div className="text-xs text-pierre font-light space-y-1 break-all">
                      {lead.clientEmail && <div>{lead.clientEmail}</div>}
                      {lead.clientPhone && <div>{lead.clientPhone}</div>}
                    </div>
                  )}
                  {(skin || obj) && (
                    <div className="flex flex-wrap gap-2 text-[10px] text-pierre uppercase tracking-wider">
                      {skin && <span className="border border-sable/40 px-2 py-0.5 capitalize">{skin}</span>}
                      {obj && <span className="border border-sable/40 px-2 py-0.5 capitalize">{obj}</span>}
                    </div>
                  )}
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-sable/10">
                    {lead.clientPhone && (
                      <a
                        href={whatsappLink(lead.clientPhone, lead.clientName ?? "")}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 min-w-[100px] text-center bg-carbone text-blanc-casse px-3 py-2 text-xs font-light tracking-wide min-h-[44px] flex items-center justify-center"
                      >
                        WhatsApp
                      </a>
                    )}
                    {lead.clientEmail && (
                      <a
                        ...
                        className="flex-1 min-w-[100px] text-center border border-sable text-terre px-3 py-2 text-xs font-light tracking-wide min-h-[44px] flex items-center justify-center"
                      >
                        E-mail
                      </a>
                    )}
